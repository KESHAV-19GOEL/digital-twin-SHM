...
if __name__ == '__main__':        
    ...
    classifier_layer = keras.layers.TFSMLayer(str(Path(path_classifier)),call_endpoint="serving_default")
    regressors = [keras.models.load_model(Path(path_regressors[i])) for i in range(n_class-1)]
    
    classifier = Sequential([classifier_layer])
    #  ###############    test
    # classifier = tf.saved_model.load(Path(path_classifier))
    # regressors = [tf.saved_model.load(Path(path_regressors[i])) for i in range(n_class-1)]
